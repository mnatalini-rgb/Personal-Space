...
> **⚠️ UPDATE (March 2026):** The weighted OVU formula (`OVU = 3·AL + 6·C1 + 9·C2 + 3·C3`) documented below is **DEPRECATED**. We now use **direct € values per conversion point** instead of abstract weighted scores. The normalised rate metric is **€ per 1K EBU** (not OVU per 1K EBU). Total Partner Value is now tracked on a **30-day rolling window** (not weekly) to smooth mission activation seasonality and align with the ~1-month campaign lifecycle. The TAVI framework structure (BI + Advertising components, eligible audience normalisation) remains valid — only the scoring methodology and tracking cadence have changed. See `analysis/tradeit_nsm_analysis.py` for the current implementation.
